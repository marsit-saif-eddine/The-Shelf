...
exports.getClubDetails = async (req, res) => {
  try {
    ...
    const club = await dbClubs.findOne({
...
exports.getClubMembers = async (req, res) => {
  try {
    const dbClubs = getDb().collection("clubs");
    const members = await dbClubs
      .find({ club_id: req.query.club_id })
      .toArray();
...
exports.getClubAnnouncements = async (req, res) => {
  try {
    ...
    const clubAnnouncements = await dbClubAnnouncements
      .find({ club_id: req.query.club_id })
      .sort({ _id: -1 })
      .toArray();
...
exports.deleteAnnouncement = async (req, res) => {
  try {
    ...
    const result = await dbClubAnnouncements.deleteOne({
